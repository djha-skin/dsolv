...
(defun repo-map-to-package-infos (repo)
  "Convert an fset repository map (package-id -> seq of entry maps)
   into an fset repository map (package-id -> seq of package-info
   structs)."
  (fset:reduce
    (lambda (acc id)
      (fset:with acc id
  ...
  (let* ((initial-repository
           (if add-to
               (let* ((add-string (data-slurp add-to))
                 ...
                 (repo-map-to-package-infos (to-fset add-data)))
               (f:empty-map)))
         (dscard-files
           ...
           (reduce
             (lambda (acc card-path)
               (let* ((card-data (read-card card-path))
                 ...
                 (if pkg-id
                     (let ((pkg-info (fset-map-to-package-info card-data)))
                       (fset:with acc pkg-id
                                  (let ((existing (fset:lookup acc pkg-id)))
                                    (if existing
                                        (fset:with-last existing pkg-info)
                                        (fset:with-last (f:empty-seq) pkg-info)))))
                     acc)))
             dscard-files
         ...
         (sorted-repository
           (if sortindex
               (let ((result (f:empty-map)))
                 ...
                 result)
               merged-repository)))
    ;; Build a hash table of package-id -> list of package-info structs;
    ;; NRDL dispatches the structs to the emit-nrdl-struct methods.
    (let ((repo-ht (make-hash-table :test 'equal)))
      (fset:do-map (id packages sorted-repository)
                   (setf (gethash id repo-ht)
                         (fset:convert 'list packages)))
